---
title: "[心得] USB Audio Player PRO/USB Audio Recorder PRO介紹"
pubDate: 2014-05-28
description: "今天來寫一下，使用USB Audio Player PRO/USB Audio Recorder PRO的心得。"
tags:
  - "Hippo Cricri+"
  - "Dragonfly"
  - "USB OTG"
  - "耳機"
  - "介紹"
  - "DAC"
  - "我的吉米城"
  - "Android"
  - "USB Audio Player PRO"
  - "音響"
  - "心得"
  - "USB Audio Recorder PRO"
---
今天來寫一下，使用USB Audio Player PRO/USB Audio Recorder PRO的心得。  
  
這個軟體可以讓大部份支援USB OTG的手機/平板使用大部份的USB DAC  
  
  
會想要研究這個軟體，主要是因為……大部份的Android手機或的音質真的都很令人無言……  
只有少部份超貴的或是HIFI走向的手機有去做到這部份，連SAMSUNG或HTC的手機都不見得在音質這塊有做得很好。  
不像APPLE的手機或是其他裝置，都有一定程度的音質，所以直推就很OK了，如果以隨身使用就有一定程度，而且如果真的要追求音質，也有許多設備直接支援這些APPLE系列的手機或平板……  
  
我也了解，為什麼會這樣，主要還是在於Android太廣泛被運用了，很多廠商又會自行開發一些新的規格或是設計之類的，沒有像是APPLE的那種可以說是統一的標準，所以很難被完整研發……  
  
目前我這邊知道的是，最近開始慢慢的一些手機開始支援USB OTG可以外接USB DAC的部份，像是Samsung S3、Note2之後的都慢慢有支援，HTC的M8好像也有，不過也不是所有的USB DAC都可以使用，會有一些晶片的USB DAC無法……像是舉個例子，我手上有的Dragonfly V1.0和Hippo Cricri+就無法支援了……@@  
  
而支援的部份又有分成幾種，有些是可以用該廠牌內建的播放器播放，而其他像是KKBOX、SPOTIFY的部份就無法，也有其他的是所有只要聲音都可以播放出來這樣……這就要去實際測試了，以我自己之前的測試，是使用Fiio E18在Samsung Note2上可以播放，不過常常會跳奇怪的錯誤BUG回報……可能是因為我之前升級沒有做雙WIPE吧……下次有空重新雙WIPE一下，再去測測看。  
  
好，那麼如果說各位的手機無法支援直接用USB OTG接USB DAC的話，也先不用灰心，請參考下面的說明進行測試。  
  
\=======================================================================  
                         說  明  開  始  
\=======================================================================  
一、大家可以先查照下面這個網址：  
  [http://www.extreamsd.com/USBAudioRecorderPRO/](http://www.extreamsd.com/USBAudioRecorderPRO/)  
  這是USB Audio Player PRO/USB Audio Recorder PRO的官方網站，  
  上面有寫一些注意事項，像是不要買那種光轉接頭的東西，  
  而是要買真正OTG線材這一類的，也有相關的USB音效裝置還有手機的支援清單，  
  大家可以先查一下^^  
  
二、確定清單上面有你的裝置可以執行之後，心臟大的可以直接購買@@  
  不過我個人建議大家先到下面：  
  a.Google Play商城中的USB Audio Player PRO  
    [https://play.google.com/store/apps/details?id=com.extreamsd.usbaudioplayerpro](https://play.google.com/store/apps/details?id=com.extreamsd.usbaudioplayerpro)  
    裡面有free trial的下載地點，點進去下載，  
    不過剛剛我測試，是無法下載，大家可以使用b方式  
  b.Google Play商城中的USB Audio Recorder PRO  
    [https://play.google.com/store/apps/details?id=com.extreamsd.usbaudiorecorderpro](https://play.google.com/store/apps/details?id=com.extreamsd.usbaudiorecorderpro)  
    裡面也有TRIAL version的部份，下載後，進行安裝  
    不過安裝之前，記得到手機「設定」中的「安全性」部份，  
    「勾選」允許安裝「未知的來源」這樣，  
    當然這樣安全性會較低，不過為了測試，這沒有辦法，各位可以在測試開關閉此選項。  
  
三、進行測試之前，先說明一下兩種版本的差別.  
  USB Audio Player PRO主要是為了「播放」而生，  
  基本上可以播放wav/flac/ogg/mp3/aiff/aac/m4a/ape……等，連24BIT跟32BIT都支援，很強大，  
  而且介面也比較像是個播放器，還可以：  
    a.「音量軟體/硬體控制」(重要)，USB Audio Recorder PRO無法軟體控制音量，  
    如果使用的DAC無音量旋鈕會很痛苦……@@  
    b.「無縫播放」(重要)，因為USB Audio Recorder PRO是無法無縫播放，  
    變成每一首曲子會隔好一陣子，差不多30秒……  
  而USB Audio Recorder PRO，則是為了「錄音」而生，  
  播放功能對他而言只是為了確定錄音內容而存在，相對陽春，  
  不過如果有錄音需求，他還是不二人選……  
  
四、測試步驟：  
  a.準備好USB OTG線、USB DAC，如下圖，  
  下面我使用了兩台隨身DAC/耳擴來測試，分別是Dragonfly V1.0、Hippo Cricri+  

[![](/blog/img/20140528_205039.webp)](/blog/img/20140528_205039.webp)

  
  b.開始裝置電源，先插入USB OTG線中(重要)，因為這樣才容易抓到該DAC。  
  c.再將USB OTG線插入手機的MicroUSB孔中，應會顯示插入USB裝置，如下圖  
  

[![](/blog/img/Screenshot_2014-05-28-20-52-52.webp)](/blog/img/Screenshot_2014-05-28-20-52-52.webp)

  d.開啟USB Audio Player PRO/USB Audio Recorder PRO軟體  
  
五、下面先以USB Audio Player PRO做介紹，之後會再說明USB Audio Recorder PRO部份  
  a.會出現「允許應用程度USB Audio Player PRO存取USB裝置嗎？  
  請按「確認」(依需求可以勾選「依預設使用此USB裝置」)  

[![](/blog/img/Screenshot_2014-05-28-20-54-31.webp)](/blog/img/Screenshot_2014-05-28-20-54-31.webp)

  
  b.進入播放畫面如下圖，下方為常見的播放、停止、循環、調整歌曲時間等功能  

[![](/blog/img/Screenshot_2014-05-28-20-48-05.webp)](/blog/img/Screenshot_2014-05-28-20-48-05.webp)

  
  c.播放畫面右邊有個奇怪的銀色鈕，為音量調整，往上為大聲，往下為小聲，  
    預設為軟體音量控制，但是也可取消或改為硬體音量控制等，  
    需要進入設定(上方偏右的按鈕)，Perfs中有個Volume Control更改，  
    像是Dragonfly如果覺得軟體控制的最大聲還不夠的話，可以改成硬體控制，如下圖  

[![](/blog/img/Screenshot_2014-05-28-20-55-12.webp)](/blog/img/Screenshot_2014-05-28-20-55-12.webp)

  
  d.於播放畫面的上方偏左按鈕，可開啟音樂，  
    我個人都是使用資料夾播放(第二個選項，如下圖)  

[![](/blog/img/Screenshot_2014-05-28-20-57-06.webp)](/blog/img/Screenshot_2014-05-28-20-57-06.webp)

  
  e.開啟音樂試播，如果有爆音，或是奇怪的聲音，可先試著調Buffer大小，  
    一樣進入設定，Perfs中有個Buffer size，都調整看看(如下圖)，  

[![](/blog/img/Screenshot_2014-05-28-20-55-37.webp)](/blog/img/Screenshot_2014-05-28-20-55-37.webp)

  
    如果還是不行，將Perfs中的Force 1 packet per transfer「勾選」起來，  
    再不行，試著把Buffer調小，再「勾選」USB teak 1，  
    還不行，又或是發現可能手機效能不足，可以使用「Force 16-bit」這樣。  
    上面這樣調整都還不行，寫信問開發者吧@@  
  
  f.在設定當中有個EQ部份，如下圖，大家有空還可以玩玩看，我個人是不玩這個的XD  

[![](/blog/img/Screenshot_2014-05-28-20-55-20.webp)](/blog/img/Screenshot_2014-05-28-20-55-20.webp)

  
  e.於播放時候，會在上面功能表有顯示，可以直接點入進行調整，  
    這也是USB Audio Recorder PRO沒有的……  

[![](/blog/img/Screenshot_2014-05-28-20-56-48.webp)](/blog/img/Screenshot_2014-05-28-20-56-48.webp)

  
六、USB Audio Recorder PRO部份  
  a.會出現「允許應用程度USB Audio Recorder PRO存取USB裝置嗎？  
  請按「確認」(依需求可以勾選「依預設使用此USB裝置」)  
  

[![](/blog/img/Screenshot_2014-05-28-20-58-06.webp)](/blog/img/Screenshot_2014-05-28-20-58-06.webp)

  b.進入播放畫面(MAIN)如下圖  

[![](/blog/img/Screenshot_2014-05-28-21-02-05.webp)](/blog/img/Screenshot_2014-05-28-21-02-05.webp)

  
  c.因為以錄音為主，所以畫面左上角有個資料夾可以開啟音效檔(WAV、AIFF、OGG等)，  
    或是改變錄音存放路徑/名稱這樣  
    下方有兩聲道部份收音/放音的狀況顯示，也有拉桿可調整檔案時間，  
    不過在此畫面下，無法連續播放  
    重點為無法調整音量……所以只有Hippo Cricri+可以比較正常使用= =  
  
  d.I/O的部份如下圖，為檔案存入跟輸出的格式，  
    基本上也沒有什麼好選的，他也會自動選好。  
    只有Buffer部份，跟上面一樣可以調整，  
    我個人聽感是愈小的Buffer音質愈好，大家可以再試一下。  

[![](/blog/img/Screenshot_2014-05-28-20-59-16.webp)](/blog/img/Screenshot_2014-05-28-20-59-16.webp)

  
  e.Play List部份，為簡易版本的連續播放器，  
    基本上，選好你要播放的檔案，在此頁面按播放，  
    他自動會依序播放其資料夾的其他檔案，不過，中間都會隔30秒……= =  

[![](/blog/img/Screenshot_2014-05-28-21-03-33.webp)](/blog/img/Screenshot_2014-05-28-21-03-33.webp)

  

七、結論

  如果各位真的想要有好音質，不在乎多個接來接去的東西，又剛好身邊的USB DAC有支援USB Audio Player PRO的話，我真的蠻建議大家來使用看看的！音質會有很明顯的提升，比光用耳擴好上許多喔！

  

有什麼設定或是使用上面的問題，也都可以再跟我討論@@


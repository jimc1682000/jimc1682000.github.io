...
| **自 host** | 三個家族皆自 host subset（`public/fonts/`，由 `scripts/build-fonts.mjs` 產出）：思源宋體 400/700、思源黑體 400/500、Noto Sans Mono 400。`@font-face` inline 在 `BaseHead.astro`，`font-display: swap` + 度量覆寫備援 face |
| **字重限制** | 思源黑體**沒有 600**（只有 …/400/500/700/…）。sans 元素要重量時用 `500`；serif 的 `600` 刻意保留，會落到我們有的 700 |
...
| 2026-07-30 | **三個字型家族全部改為自 host subset webfont**（原本全靠訪客系統字型） | 原本 `--font-serif` 點名 `Noto Serif TC` 但站上沒附檔，macOS 落到 Songti、Windows／Android 落到別的字面，**中文粗體幾乎看不出來**（Songti 的 bold 天生輕）。改為自 host 思源宋體／思源黑體／Noto Sans Mono。**實測過的取捨**：① woff2 內部已是 Brotli，HTTP 再壓縮 +0.00%，不必調 CDN 壓縮；② 可變字型反而更大（557 KB vs 兩靜態檔 464 KB）；③ 按字頻分片反而更大（4 片單頁全中 411 KB > 單檔 371 KB）；④ 有效的是 **core/rest 兩層 `unicode-range`**：英文頁只抓 108 KB、中文頁 1.31 MB。core 用精確範圍**後**宣告、rest 用寬 CJK 區塊**先**宣告 —— 實測同 family 兩 face 都涵蓋某字時後宣告者贏且前者不下載，省下逐一列碼位的 9.3 KB／face inline CSS。**產物 commit 進 repo**（CI 是 Node-only、fontTools 是 Python；原檔 84 MB 不進 git；工具版本變動會讓 hash 漂掉快取），代價由 `postbuild` 的 `check-fonts.mjs` gate 補掉 —— 缺字會讓 build 失敗而非靜默掉字。`font-display: swap` 配度量覆寫 face（`ascent-override: 115.1%` 等，數字取自思源宋體 hhea）壓 CLS；該 face 必須用 `local()`（描述符只能掛 `@font-face`），這與「主要字型不用 `local()`」不衝突。侷限：覆寫值只對 macOS 的 Songti 準；站外迴響顯示他站內容，字元不可預測，任何 subset 都蓋不到 |
